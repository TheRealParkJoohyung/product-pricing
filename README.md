...
- **웹상에 출력** : Chat GPT API를 사용해 사용자의 질문과 질문에 대한 답변을 웹상에 출력합니다.
...
프로젝트의 기한이 마무리 되어 모두 반영하지 못한 부분을 글로 써놓고 마무리 하겠습니다.

## 9. 개선점
### 데이터 베이스
- 데이터베이스에 사용자의 입력 값을 저장한다음 추후에 다시 검색하지 않고도 비슷한 물건을 바로 확인 가능하게끔 구현 개선 필요
### 백엔드 부분
- 지금은 언어 번역 기능을 JS파일에 직접 입력하여 작성하였지만 백엔드 부분을 추가하여 언어 번역 기능을 구현 개선 필요
### API
- 중고마켓의 API로 홈 화면에 인기있는 카테고리의 정보를 좌 / 우측에 띄우는 구현 필요
### 사용자 경험
- 계산 결과를 기다리는 동안 모래시계 그림 혹은 텍스트를 설정하여 입력을 잘 받았으며 계산중이라는 것을 알려주는 사용자 경험 개선 필요
